...
Shader PreviewCanvas::_texshader;
GLint PreviewCanvas::_texshader_uniform_texture;
...
void PreviewCanvas::setViewport(float aspect) {
  ...
  if (aspect > (cbx - cay) * 1.0 / (cby - cay)) {
    ...
    glViewport((cax + cbx) / 2.0 - (cby - cay) * aspect / (2.0), cax, (cby - cay) * aspect, cby - cay);
  }
  glEnable(GL_BLEND);
...
void PreviewCanvas::render(set<key_location>& down) {
  ...
  shared_ptr<Filter_Resource_Output> result = _inputs["result"]->_binding.lock();
  shared_ptr<Filter_Resource_ColorBuffer> buffer = result->castTo<Filter_Resource_ColorBuffer>();
  shared_ptr<Filter> filter = result->_filter.lock();
  filter->calculate(0);
  float arr[12] = {
      -1.0f, -1.0f,
      -1.0f, +1.0f,
  ...
  glBindTexture(GL_TEXTURE_2D, buffer->_source->_col_attachments[buffer->_col_att_id]);
  glUniform1i(_texshader_uniform_texture, 0);
  ...
  //_enc->addFrame(result, _frameData);
