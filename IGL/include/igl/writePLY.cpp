...
IGL_INLINE bool igl::writePLY(
  ...
  const Eigen::MatrixBase<DerivedCOL> & VC,
  const Eigen::MatrixBase<DerivedCOL> & FC,
  const bool ascii)
{
  ...
  typedef typename DerivedCOL::Scalar COLScalar;
  ...
  { /* list of property information for a vertex */
    ...
    {"red", ply_type<COLScalar>(),ply_type<COLScalar>(),offsetof(Vertex,r),0,0,0,0 },
    {"green", ply_type<COLScalar>(),ply_type<COLScalar>(),offsetof(Vertex,g),0,0,0,0 },
    {"blue", ply_type<COLScalar>(),ply_type<COLScalar>(),offsetof(Vertex,b),0,0,0,0 },
  };
  ...
  { /* list of property information for a face */
    ...
    { "red", ply_type<COLScalar>(),ply_type<COLScalar>(),offsetof(Face,r),0,0,0,0 },
    { "green", ply_type<COLScalar>(),ply_type<COLScalar>(),offsetof(Face,g),0,0,0,0 },
    { "blue", ply_type<COLScalar>(),ply_type<COLScalar>(),offsetof(Face,b),0,0,0,0 },
  };
  const bool has_normals = N.rows() > 0;
  const bool has_texture_coords = UV.rows() > 0;
  const bool has_vertex_colors = VC.rows() > 0;
  const bool has_face_colors = FC.rows() > 0;
  ...
  plist.clear();
  plist.push_back(face_props[0]);
  if (has_face_colors) {
    plist.push_back(face_props[1]);
    plist.push_back(face_props[2]);
    plist.push_back(face_props[3]);
  }
  ply_describe_element(ply, "face", F.rows(), plist.size(),
    &plist[0]);
...
IGL_INLINE bool igl::writePLY(
  ...
  const bool ascii)
{
  Eigen::Matrix<typename DerivedV::Scalar,Eigen::Dynamic,Eigen::Dynamic> N,UV,VC,FC;
  return writePLY(filename,V,F,N,UV,VC,FC,ascii);
}
