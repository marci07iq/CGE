...
template <
  ...
  std::vector<std::vector<COLtype> >  & VC,
  std::vector<std::vector<COLtype> >  & FC)
{
  using namespace std;
  // Largely follows ply2iv.c
  FILE * ply_file = fopen(filename.c_str(),"rb");
  if(ply_file == NULL)
  {
    return false;
  }
  return readPLY(ply_file,V,F,N,UV,VC,FC);
}
  ...
  std::vector<std::vector<COLtype> >  & FC)
{
  ...
  igl::ply::PlyProperty face_props[] = { /* list of property information for a face */
    ...
    { "red", PLY_DOUBLE, PLY_DOUBLE, offsetof(Face,r), 0, 0, 0, 0 },
    { "green", PLY_DOUBLE, PLY_DOUBLE, offsetof(Face,g), 0, 0, 0, 0 },
    { "blue", PLY_DOUBLE, PLY_DOUBLE, offsetof(Face,b), 0, 0, 0, 0 },
  };
  ...
  if (plist != NULL)
  {
    ...
    {
      VC.resize(elem_count,std::vector<COLtype>(3));
    }else
    {
      VC.resize(0);
    }
  ...
  if (plist != NULL)
  {
    ...
    for (int j = 0; j < nprops; j++) {
      ...
      if (igl::ply::equal_strings("red", prop->name)
        || igl::ply::equal_strings("green", prop->name)
        || igl::ply::equal_strings("blue", prop->name)) {
        ply_get_property(in_ply, "face", &face_props[1]);
        ply_get_property(in_ply, "face", &face_props[2]);
        ply_get_property(in_ply, "face", &face_props[3]);
    ...
    F.resize(elem_count);
    if (has_face_colors) {
      FC.resize(elem_count, std::vector<COLtype>(3));
    } else {
      FC.resize(0);
    }
...
IGL_INLINE bool igl::readPLY(
  ...
  Eigen::PlainObjectBase<DerivedCol> & VC,
  Eigen::PlainObjectBase<DerivedCol> & FC)
{
  ...
  std::vector<std::vector<typename DerivedCol::Scalar> > vVC;
  std::vector<std::vector<typename DerivedCol::Scalar> > vFC;
  if(!readPLY(filename,vV,vF,vN,vUV, vVC, vFC))
  {
    return false;
  }
  return 
    ...
    list_to_matrix(vVC,VC) &&
    list_to_matrix(vFC,FC);
}
...
{
  Eigen::MatrixXd N, UV;
  Eigen::MatrixXd VC, FC;
  return readPLY(filename,V,F,N,UV,VC,FC);
}
